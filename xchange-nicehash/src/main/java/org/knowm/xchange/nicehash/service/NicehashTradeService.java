...
public class NicehashTradeService extends NicehashTradeServiceRaw implements TradeService {
  ...
  public interface NicehashOrderFlags extends IOrderFlags {
  ...
  public OpenOrders getOpenOrders(OpenOrdersParams params) throws IOException {
    try {
      ...
      nicehashOpenOrders.forEach(
          nicehashOrder -> {
            Order order = NicehashAdapters.adaptOrder(nicehashOrder);
  ...
  private String getClientOrderId(Order order) {
    ...
    for (IOrderFlags flags : order.getOrderFlags()) {
      if (flags instanceof NicehashOrderFlags) {
        NicehashOrderFlags bof = (NicehashOrderFlags) flags;
  ...
  public UserTrades getTradeHistory(TradeHistoryParams params) throws IOException {
    try {
      ...
      Long recvWindow =
          (Long)
              exchange.getExchangeSpecification().getExchangeSpecificParametersItem("recvWindow");
      List<NicehashTrade> nicehashTrades =
      ...
      long lastId = nicehashTrades.stream().map(t -> t.id).max(Long::compareTo).orElse(0L);
  ...
  public TradeHistoryParams createTradeHistoryParams() {
    ...
    // return new NicehashTradeHistoryParams();
    return null;
  }
